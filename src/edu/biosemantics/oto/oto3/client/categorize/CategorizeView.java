...
import edu.biosemantics.oto.oto3.shared.model.TermProperties;
...
import edu.biosemantics.oto.oto3.client.categorize.DndDropEventExtractor;
...
public class CategorizeView extends BorderLayoutContainer implements IsWidget {
	...
	public class CategoryTreeNode extends TextTreeNode {
	...
	public class CategoriesView extends PortalLayoutContainer {
		...
		public void setCategories(List<Category> categories) {
			...
			for(Category category : categories) {
				CategoryPortlet categoryPortlet = new CategoryPortlet(eventBus, category);
	...
	public class TermsView extends TabPanel {
		...
		private Map<Category, CategoryTreeNode> categoryCategoryTreeNodeMap;
		...
		private void bindEvents() {
			eventBus.addHandler(TermUncategorizeEvent.TYPE, new TermUncategorizeEvent.TermUncategorizeHandler() {
				...
				public void onUncategorize(List<Term> terms, Category oldCategory) {
					for(Term term : terms) {
						treeStore.add(categoryCategoryTreeNodeMap.get(term.getInitialCategory()), termTermTreeNodeMap.get(term));
						listStore.add(term);
					}
				}
			});
			...
			dropTarget.addDropHandler(new DndDropHandler() {
				...
				public void onDrop(DndDropEvent event) {
					...
					if(DndDropEventExtractor.isSourceCategoryPortlet(event)) {
						eventBus.fireEvent(new TermUncategorizeEvent(DndDropEventExtractor.getTerms(event), DndDropEventExtractor.getCategoryPortletSource(event).getCategory()));
					}
				}
			});
		}
		
		public void setInitialCategories(List<Category> categories) {
			...
			for(Category category : categories) {
				...
				categoryCategoryTreeNodeMap.put(category, categoryTreeNode);
				treeStore.add(categoryTreeNode);
				for(Term term : category.getTerms()) {
					TermTreeNode termTreeNode = new TermTreeNode(term);
					termTermTreeNodeMap.put(term, termTreeNode);
					treeStore.add(categoryTreeNode, termTreeNode);
					listStore.add(term);
				}
			}
		}
	...
	public void setInitialCategories(List<Category> categories) {
		termsView.setInitialCategories(categories);
	}
