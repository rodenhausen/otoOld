...
import edu.biosemantics.oto.oto3.client.categorize.event.CategorizeCopyTermEvent;
import edu.biosemantics.oto.oto3.client.categorize.event.CategorizeMoveTermEvent;
import edu.biosemantics.oto.oto3.client.categorize.event.TermCategorizeEvent;
import edu.biosemantics.oto.oto3.client.categorize.event.TermSelectEvent;
import edu.biosemantics.oto.oto3.client.categorize.event.TermUncategorizeEvent;
...
import edu.biosemantics.oto.oto3.shared.model.TextTreeNode;
import edu.biosemantics.oto.oto3.shared.model.TextTreeNodeProperties;

public class CategoryPortlet extends Portlet {
	...
	public CategoryPortlet(EventBus eventBus, Category category) {
		this.eventBus = eventBus;
		this.category = category;
		this.setHeadingText(category.getText());
	...
	private void bindEvents() {
		eventBus.addHandler(CategorizeCopyTermEvent.TYPE, new CategorizeCopyTermEvent.CategorizeCopyTermHandler() {
			@Override
			public void onCategorize(List<Term> terms, Category sourceCategory, Category targetCategory) {
		...
		eventBus.addHandler(CategorizeMoveTermEvent.TYPE, new CategorizeMoveTermEvent.CategorizeMoveTermHandler() {
			...
			public void onCategorize(List<Term> terms, Category sourceCategory,	Category targetCategory) {
				...
				if(sourceCategory.equals(category)) {
					for(Term term : terms) {
		...
		eventBus.addHandler(TermCategorizeEvent.TYPE, new TermCategorizeEvent.TermCategorizeHandler() {
			@Override
			public void onCategorize(List<Term> terms, Category category) {
				if(CategoryPortlet.this.category.equals(category)) {
					for(Term term : terms)
						addToStore(new MainTermTreeNode(term));
				}
			}
		...
		eventBus.addHandler(TermUncategorizeEvent.TYPE, new TermUncategorizeEvent.TermUncategorizeHandler() {
			@Override
			public void onUncategorize(List<Term> terms, Category oldCategory) {
				if(CategoryPortlet.this.category.equals(category)) {
	...
	private void setupDnD() {
		...
		dropTarget.addDropHandler(new DndDropHandler() {
			...
			public void onDrop(DndDropEvent event) {
				...
				if(DndDropEventExtractor.isSourceCategoryPortlet(event)) {
			...
			private void onDnd(final DndDropEvent dropEvent, DropSource source) {
				switch(source) {
				case INIT:
					eventBus.fireEvent(new TermCategorizeEvent(DndDropEventExtractor.getTerms(dropEvent), category));
					break;
				case PORTLET:
					Menu menu = new CopyMoveMenu(new SelectionHandler<Item>() {
						@Override
						public void onSelection(SelectionEvent<Item> event) {
							CategoryPortlet sourcePortlet = DndDropEventExtractor.getCategoryPortletSource(dropEvent);
							eventBus.fireEvent(new CategorizeCopyTermEvent(DndDropEventExtractor.getTerms(dropEvent), sourcePortlet.getCategory(), category));
						}
					}, new SelectionHandler<Item>() {
						@Override
						public void onSelection(SelectionEvent<Item> event) {
							CategoryPortlet sourcePortlet = DndDropEventExtractor.getCategoryPortletSource(dropEvent);
							eventBus.fireEvent(new CategorizeMoveTermEvent(DndDropEventExtractor.getTerms(dropEvent), sourcePortlet.getCategory(), category));
						}
					});
					menu.show(CategoryPortlet.this);
					break;
				default:
					break;
				}
